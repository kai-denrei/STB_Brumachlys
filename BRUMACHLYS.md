...
> A simultaneous-turn tactical wargame in the Weewar / Zetawar lineage.

---

## 0. TL;DR for the implementing agent

Build a single-page React + TypeScript hot-seat PoC of a hex-grid wargame where both players commit orders blind, then a deterministic resolver plays them out in **initiative order**. Combat math is ported directly from Weewar (see `Appendix A`) and Zetawar (MIT-licensed reference, see `Appendix B`). Map format is Weewar XML (sample provided).
